...
public class persen extends Activity
{
    ...
    protected void onCreate(Bundle savedInstanceState)
    {
        ...
        setContentView(R.layout.persen);
	...
	public void hitung_persen(View view)
	{
		...
		try
		{
			...
			if (test.isChecked())
			{
				...
				int hitung1 = 100 - hitung;
				int hitung2 = ((data2 * hitung1) / data1);
				String mulai = String.format("x = 100 - (%01d + %01d + %01d + %01d + %01d )", per1, per2, per3, per4, per5);
			...
			else
			{
				...
				int hitung = per1 + per2 + per3 + per4 + per5;
				int hitung1 = 100 - hitung;
				int hitung2 = ((data3 * hitung1) / 100);
				String mulai = String.format("x = 100 - (%01d + %01d + %01d + %01d + %01d )", per1, per2, per3, per4, per5);
				String mulai1 = String.format("\nx = 100 - %01d", hitung);
				String mulai2 = String.format("\nx = %01d", hitung1);
				String mulai3 = String.format("\nx = (%01d / 100) x %01d", hitung1, data3);
				String mulai4 = String.format("\nx = %01d", hitung2);
